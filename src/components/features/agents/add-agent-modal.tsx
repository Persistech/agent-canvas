...
import {
  ADD_AGENT_DOCS_URL,
  ADD_AGENT_EXAMPLE_FILE,
} from "#/constants/agents-docs";
...
const ADD_AGENT_STEP_KEYS = [
  I18nKey.SETTINGS$AGENTS_ADD_MODAL_STEP_1,
  I18nKey.SETTINGS$AGENTS_ADD_MODAL_STEP_2,
  I18nKey.SETTINGS$AGENTS_ADD_MODAL_STEP_3,
] as const;

const ADD_AGENT_INLINE_CODE_COMPONENTS = {
  path: <InlineCodeChip />,
};
...
function AddAgentExampleBlock() {
  const { t } = useTranslation("openhands");
  const [copied, setCopied] = React.useState(false);

  const handleCopy = async () => {
    await navigator.clipboard.writeText(ADD_AGENT_EXAMPLE_FILE);
    setCopied(true);
  };
  ...
  return (
    <div className="relative">
      <pre
        data-testid="add-agent-modal-example"
        className={cn(
      ...
      <button
        type="button"
        data-testid="add-agent-modal-example-copy"
...
export function AddAgentModal({ onClose }: AddAgentModalProps) {
  ...
  return (
    <ModalBackdrop
      onClose={onClose}
      aria-label={t(I18nKey.SETTINGS$AGENTS_ADD_MODAL_TITLE)}
    >
      <div
      ...
      >
        <ModalCloseButton onClose={onClose} testId="add-agent-modal-close" />
        <header className="flex-shrink-0 px-6 pb-4 pt-6">
          <h2 className={cn("pr-6", modalTitleLgClassName)}>
            {t(I18nKey.SETTINGS$AGENTS_ADD_MODAL_TITLE)}
          </h2>
          <p className="mt-4 text-sm text-tertiary-light">
            {t(I18nKey.SETTINGS$AGENTS_ADD_MODAL_INTRO)}
          </p>
        </header>
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-6 custom-scrollbar">
          <section className="flex flex-col gap-2">
            <h3 className="text-sm font-semibold text-foreground">
              {t(I18nKey.SETTINGS$AGENTS_ADD_MODAL_STORAGE_TITLE)}
            </h3>
            <p className="text-xs leading-relaxed text-tertiary-light">
              <Trans
                ns="openhands"
                i18nKey={I18nKey.SETTINGS$AGENTS_ADD_MODAL_STORAGE_BODY}
                components={ADD_AGENT_INLINE_CODE_COMPONENTS}
              />
            </p>
            <p className="text-xs text-tertiary-light">
              {t(I18nKey.SETTINGS$AGENTS_ADD_MODAL_EXAMPLE_LABEL)}
            </p>
            <AddAgentExampleBlock />
            <ol className="list-decimal space-y-1 pl-4 text-xs leading-relaxed text-tertiary-light">
              {ADD_AGENT_STEP_KEYS.map((key) => (
                <li key={key}>
                  {key === I18nKey.SETTINGS$AGENTS_ADD_MODAL_STEP_1 ? (
                    <Trans
                      ns="openhands"
                      i18nKey={key}
                      components={ADD_AGENT_INLINE_CODE_COMPONENTS}
          ...
          <p className="text-xs leading-relaxed text-tertiary-light">
            {t(I18nKey.SETTINGS$AGENTS_ADD_MODAL_RELOAD_NOTE)}
          </p>

          <a
            href={ADD_AGENT_DOCS_URL}
            target="_blank"
            rel="noreferrer"
            data-testid="add-agent-modal-docs-link"
            className="self-start text-xs text-[var(--oh-muted)] transition-colors hover:text-white hover:underline"
          >
            {t(I18nKey.SETTINGS$AGENTS_ADD_MODAL_VIEW_DOCS)}
          </a>
        </div>
        ...
        <footer className="flex flex-shrink-0 justify-end gap-2 px-6 pb-6 pt-4">
          <BrandButton
            ...
            variant="secondary"
            onClick={onClose}
            testId="add-agent-modal-dismiss"
          >
