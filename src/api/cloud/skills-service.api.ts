...
/**
 * Fetch the full list of skills from the cloud backend. The cloud endpoint
 * is paginated (page_id cursor); we walk all pages so the settings UI gets
 * a complete list in one call. The cloud SkillInfo shape
 * (name/type/source/triggers) matches the GUI's SkillInfo type, so items
