...
- Dev-mode request fanout note: HAR/CSV captures count each Vite-served ESM module as a separate request, so slow local loads are dominated by eager import breadth (`/src/**/*.ts(x)` + `/node_modules/.vite/*`) rather than production bundling. High-impact reductions came from (1) lazy-loading home/sidebar overlays that are closed on first paint (`settings-modal`, conversation panel, user menu/add-backend modal, workspace browser/manage modal, repo/workspace launcher variants), (2) removing dead route-level eager imports like the stray `PrefetchPageLinks` in `src/routes/home.tsx`, and (3) keeping icons/tooltips library-backed while funnelling app icon usage through the shared `src/components/shared/icons.tsx` re-export module and prebundling `lucide-react` / `react-icons/fa6` in Vite dev.
