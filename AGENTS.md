...
- **Test specs**:
  ...
  - `mock-llm-diff-stress.spec.ts` — Diff view responsiveness stress test and full commit lifecycle. Creates 12 files of varying sizes (1-line text up to 500-line Python modules) in a single conversation, then measures render times: diff list appearance after refresh, expanding small/medium/large diffs, and rapid sequential expand/collapse cycles. Asserts all timings stay below thresholds (30s list render, 10-20s expand depending on size, 5s per rapid cycle). File content is generated programmatically via `generatePythonContent()`. The trajectory is built dynamically from a `FILE_SPECS` manifest. Uses terminal commands (`cat > file << 'EOF'`) instead of `file_editor` to avoid needing the absolute worktree path — terminal runs in the workspace CWD. The diff view toggle is `files-tab-diff-toggle-option-on` and refresh is `files-tab-refresh`. Allows 1 missing file due to potential agent-server internal LLM call padding. After performance measurements, the test continues the same conversation through the full commit lifecycle: step 3 sends a second message that runs `git add -A && git commit` (with inline `-c user.name`/`user.email` since the worktree may lack git identity), then refreshes the diff view and verifies it shows the empty state (`"hasn't made any changes yet"` with 0 diff viewers); step 4 sends a third message that creates a new file post-commit, then verifies the diff view correctly shows the fresh change (`post_commit.py`). Uses `afterAll` (not `afterEach`) for conversation cleanup since steps 3–4 resume the conversation created in step 2.
