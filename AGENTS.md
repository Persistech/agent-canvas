...
**Stale state from earlier serial tests** — Mock-LLM tests run serially (`workers: 1`) against a real agent-server. Earlier tests (conversation, automation) persist settings on the server. If your test depends on "clean" state but a prior test configured `llm_base_url`, `llm_model`, etc., the form may render in a different view mode. Use `resetAgentServerLLMSettings(request)` from `mock-llm-helpers.ts` in a `beforeEach` hook to PATCH the agent-server settings back to defaults via the API. This is preferable to intercepting and patching `GET /api/settings` responses with `page.route()` — the API reset genuinely clears the server state so all subsequent fetches see clean data, whereas response patching is brittle (must know the exact response shape, handle LIFO route ordering, and can miss fields).
