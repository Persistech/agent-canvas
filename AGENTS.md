...
- MCP enable/disable (without removing): a server can be individually disabled so the agent can't discover/use it, while its `mcp_config` entry is left fully intact (nothing re-configured). Modeled on `disabled_skills`: a `disabled_mcp_servers: string[]` deny-list of SDK map keys persisted under `misc_settings.app_preferences` (added to `APP_PREFERENCE_FIELDS` in `settings-service.api.ts`, hoisted in `use-settings.ts`, defaulted in `services/settings.ts`, typed in `types/settings.ts`). Identity is the stable SDK map key: `parseMcpConfig` stamps each server with `sdkKey` (its `mcp_config` key), threaded through `flattenMcpConfig`/`MCPServerConfig.sdkKey` to the card; `toSdkMcpConfig`/`toMcp*Server` ignore `sdkKey` so it never leaks to the backend. **Enforcement is the load-bearing part**: `stripDisabledMcpServers()` in `agent-server-adapter.ts` removes disabled keys from the forwarded `mcp_config` in *both* `buildConfiguredOpenHandsAgentSettings` and `buildConfiguredAcpAgentSettings` — that is the single choke point that guarantees a disabled server is invisible to a conversation. UI: `InstalledServerCard`'s `CirclePlusCheckToggle` now means enable/disable (checkmark = enabled, dimmed + "Disabled" badge when off) via `useToggleMcpServer`; deletion moved fully to the `CustomServerEditor` modal (the page-level delete confirmation was removed). `useDeleteMcpServer` also prunes the deleted key from the deny-list. Servers built from a bare URL string have no `sdkKey` and are treated as always-enabled. Tests: adapter strip cases in `__tests__/api/agent-server-adapter.test.ts` ("disabled MCP servers"), toggle UX in `__tests__/routes/mcp-page.test.tsx`, persistence routing in `__tests__/api/settings-service.test.ts`, `sdkKey` in `__tests__/utils/mcp-config.test.ts`.
