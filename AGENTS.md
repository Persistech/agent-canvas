...
- **UI Extensions (VS Code–style) — versioning & updates.** The whole UI-extension system lives on the `feature/ui-extensions` branch and is flag-gated by `VITE_ENABLE_EXTENSIONS` (`EXTENSIONS_ENABLED = import.meta.env.VITE_ENABLE_EXTENSIONS === "true"`). Authoritative design/usage docs are `src/extensions/README.md` (kept current — read it first) and `docs/proposals/ui-extensions.md` ("Implementation status"). Architecture seam for install sources: a **source ref** (`npm:<pkg>[@range]`, `gh:<owner>/<repo>[/<subpath>][@range]`, or raw `https://` bundle dir) is parsed in `src/extensions/sources/ref.ts`, resolved to a pinned jsDelivr `ArtifactDescriptor` in `sources/resolve.ts`+`sources/jsdelivr.ts` (semver range → concrete version; `engines.agentCanvas` host-compat checked against `AGENT_CANVAS_HOST_VERSION` in `engines.ts` at the consent boundary), then acquired (`format: "dir"` today; `"zip"` reserved for a future `registry:`). The *resolved pinned* base URL + `sourceRef` + `version` + granted capabilities are persisted to `localStorage` (never code) via `installed-store.ts`/`installed-persistence.ts`. Update channel: `checkForUpdate(id)` / `updateExtension(id)` live on the extension context (`src/components/providers/extension-manager-provider.tsx`, hook `useExtensionContext`); `checkForUpdate` re-resolves the stored ref within its recorded range and reports a newer pinned artifact (a `url:` source has no update channel), and `updateExtension` applies it but **non-destructively refuses** (leaving the running version intact) when the new version is host-incompatible or requests capabilities beyond those already granted — the caller must re-run the Add/consent flow for capability growth, so authors should bump a **major** for new caps. UI surfacing: `routes/extensions.tsx` polls `checkForUpdate` per install when the installed set changes and threads `update`/`onUpdate`/`isUpdating` into `installed-extension-card.tsx` (source-ref line, "Update available" badge, Update button); the Add modal (`add-extension-modal.tsx`) shows a source-format help line and each marketplace listing's resolved install source. Marketplace catalog parses versioned refs in `src/extensions/marketplace/catalog.ts` (`EntrySource` accepts string or `{source:"npm"|"gh", ...}` object form); UI extensions are listed under a dedicated `uiExtensions` array in `.plugin/marketplace.json`, **never** `plugins`. Sample bundle + npm-ready `package.json`: `examples/extensions/hello-sidebar/`; example catalog with all three ref forms: `examples/extensions/.plugin/marketplace.json` (guarded by `__tests__/extensions/example-hello.test.ts`).

- **i18n gotchas when adding EXTENSIONS keys (learned the hard way).** (1) `src/i18n/declaration.ts` is **gitignored and generated** from `translation.json` by `scripts/make-i18n-translations.cjs` (run by `npm test`/build) — editing it by hand works locally but is never committed; only `translation.json` is the source of truth, and `tsc` will catch a duplicate enum if you re-add an existing key. Before assuming a key name is free, `grep` it: e.g. `EXTENSIONS$SOURCE_LABEL` is already "Bundle URL" (the Add-modal field), so the installed-card source line needed a distinct `EXTENSIONS$INSTALLED_SOURCE`. (2) Every new key must have all 15 locales or `__tests__/i18n/translation-completeness.test.ts` fails; values must also differ per language unless the key is in `IDENTICAL_VALUE_ALLOWLIST` in `scripts/check-translation-completeness.cjs` (the `EXTENSIONS$SOURCE_PLACEHOLDER` / `MARKETPLACE_PLACEHOLDER` example strings are allowlisted — keep them identical or add new identical-everywhere keys to that set). (3) When bulk-editing `translation.json` programmatically, round-trip with Python `json` using `OrderedDict` + `ensure_ascii=False, indent=2` and re-append a trailing newline to keep the diff minimal. (4) Tests run i18n in a mode where `t()` returns the **raw key**, so assert on `data-testid`/`title` attributes (or the untranslated `extension.name`) rather than interpolated copy. (5) Recurring hazard: editing docs/JSON with em dashes (—) or arrows (→) has previously corrupted bytes; prefer ASCII (`-`, `->`) in code/attribute literals and verify with a quick `python3 -c "open(f,encoding='utf-8').read()"` after large edits.

- Pushing to `feature/ui-extensions` in this workspace was done via the authenticated URL (`git push https://x-access-token:$GITHUB_TOKEN@github.com/OpenHands/agent-canvas.git feature/ui-extensions`) rather than `origin`, so the local `origin/feature/ui-extensions` tracking ref goes stale and `git status` shows a misleading "ahead N". The remote is actually up to date — run `git fetch origin` to reconcile before trusting ahead/behind counts.
