...
describe("AgentServerConversationService cloud branch", () => {
  it("createConversation POSTs the cloud payload directly and returns a WORKING task", async () => {
    ...
    expect(sent.selected_repository).toBe("user/repo");
    expect(sent.selected_branch).toBe("main");
    expect(sent.git_provider).toBe("github");
    expect(sent.title).toBe("Optional title");
    expect(sent.initial_message).toEqual({
      role: "user",
      content: [{ type: "text", text: "fix the bug" }],
    });
    // The local-only encrypted-settings keys must NOT be present.
    expect(sent).not.toHaveProperty("agent_settings_encrypted");
    expect(sent).not.toHaveProperty("conversation_settings_encrypted");
